...
function handleKeyPress(e) {
    ...
    if (e.keyCode === 13 || e.key === 'Enter') {
        console.log('🔍 ENTER KEY 13 DETECTED!', {
            keyCode: e.keyCode,
            key: e.key
        });
        ...
        if (analyzerWindow && analyzerWindow.style.visibility === 'hidden') {
            e.preventDefault();
            console.log('🚀 OPENING ANALYZER FROM ENTER KEY!');
            openAnalyzer();
        } else if (analyzerWindow && analyzerWindow.style.visibility === 'visible' && fileInput) {
            e.preventDefault();
    ...
    if (e.keyCode === 27 || e.key === 'Escape') {
        console.log('❌ ESCAPE KEY 27 DETECTED!', {
            keyCode: e.keyCode,
            key: e.key
        });
        const analyzerWindow = document.getElementById('analyzerWindow');
        if (analyzerWindow && analyzerWindow.style.visibility === 'visible') {
    ...
    if (e.keyCode === 32 || e.key === ' ') {
        console.log('⌨️ SPACE KEY 32 DETECTED!', {
            keyCode: e.keyCode,
            key: e.key
        });
        const analyzerWindow = document.getElementById('analyzerWindow');
    ...
    if ((e.keyCode === 70 || e.key === 'f') && (e.ctrlKey || e.metaKey)) {
        ...
        if (analyzerWindow && analyzerWindow.style.visibility === 'visible' && fileInput) {
            e.preventDefault();
